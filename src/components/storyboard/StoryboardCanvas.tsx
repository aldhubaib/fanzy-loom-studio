  ...
  return (
    <div className="h-full w-full relative bg-background overflow-hidden">
                    ...
                    <TooltipProvider delayDuration={200}>
                      ...
                      {frame.location && locationImages[frame.location] ? (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            const locKeys = Object.keys(locationImages);
                            const idx = locKeys.indexOf(frame.location!);
                            setLocationGallery({ frameId: frame.id, index: idx >= 0 ? idx : 0 });
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setLocationGallery({ frameId: frame.id, index: 0 });
                          }}
                        >
      ...
      {locationGallery && (() => {
        ...
        const currentIdx = locationGallery.index;
        const current = locEntries[currentIdx];
        if (!current) return null;
        const frame = frames.find(f => f.id === locationGallery.frameId);
        const isSelected = frame?.location === current[0];
        return (
          <div className="fixed inset-0 z-[100] bg-black/90 flex" onClick={() => setLocationGallery(null)}>
            <div className="flex-1 flex items-center justify-center relative" onClick={(e) => e.stopPropagation()}>
              <button className="absolute top-4 right-4 w-8 h-8 rounded-lg bg-background/20 hover:bg-background/40 text-foreground flex items-center justify-center transition-colors" onClick={() => setLocationGallery(null)}>
                <X className="w-5 h-5" />
              </button>
              <div className="absolute top-4 left-4 flex items-center gap-3">
                <span className="bg-background/20 text-foreground text-sm font-bold px-3 py-1 rounded-lg">
                  {currentIdx + 1} / {locEntries.length}
                </span>
              ...
              {currentIdx > 0 && (
                <button className="absolute left-4 w-10 h-10 rounded-full bg-background/20 hover:bg-background/40 text-foreground flex items-center justify-center transition-colors" onClick={() => setLocationGallery({ ...locationGallery, index: currentIdx - 1 })}>
                  <ChevronLeft className="w-5 h-5" />
                </button>
              )}
              <img src={current[1]} alt={current[0]} className="max-w-[80%] max-h-[85vh] object-contain rounded-lg" />
              {currentIdx < locEntries.length - 1 && (
                <button className="absolute right-4 w-10 h-10 rounded-full bg-background/20 hover:bg-background/40 text-foreground flex items-center justify-center transition-colors" onClick={() => setLocationGallery({ ...locationGallery, index: currentIdx + 1 })}>
                  <ChevronRight className="w-5 h-5" />
                </button>
              )}
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3">
                <span className="bg-background/30 backdrop-blur-sm text-foreground text-xs px-4 py-2 rounded-lg">
                  {current[0]}
                </span>
                <button
                  className={cn(
                  ...
                  onClick={() => {
                    setFrames(prev => prev.map(f => f.id === locationGallery.frameId ? { ...f, location: isSelected ? undefined : current[0] } : f));
                  }}
                >
            ...
            <div className="w-[180px] bg-card/50 backdrop-blur-sm border-l border-border overflow-y-auto p-2 flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
              ...
              <button
                ...
                onClick={() => {
                  setFrames(prev => prev.map(f => f.id === locationGallery.frameId ? { ...f, location: undefined } : f));
                  setLocationGallery(null);
                }}
              >
              ...
              {locEntries.map(([name, img], idx) => {
                const isActive = idx === currentIdx;
                const isSel = frame?.location === name;
                return (
                  <button key={name} className={`relative rounded-lg overflow-hidden border-2 transition-all ${isActive ? "border-primary" : "border-transparent hover:border-muted-foreground/40"}`} onClick={() => setLocationGallery({ ...locationGallery, index: idx })}>
